...
同一台实盘终端，`100nMilliSecond`，每种组合**重启策略、等日志 cadence 回到
...
延迟和 zmq 同档）。装不了 redis 用 **zmq + drain**；都装不了用 **mysql** 兜底。
都别开后台线程。
...
**cfquant 的 92.8ms 与本项目 redis 的 3.8ms 差 24 倍**，差距不在实现好坏，在传输
选型 —— 它走命名管道，本项目默认走 redis。

它也不是管道本身的极限：拿回显桩单测传输层，cfquant 0.822ms、本项目 0.207ms
（它多一跳 hub），管道裸往返只要 0.012ms。也就是说 92ms 里有 99% 花在
**QMT 侧的唤醒调度**上，跟线缆无关。
